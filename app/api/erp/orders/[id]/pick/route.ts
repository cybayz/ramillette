...
export async function POST(request: Request, { params }: Props) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: `Order #${order.orderNumber} marked as picked`,
      order: updated,
    });
  } catch (error: any) {
    console.error("Order picking error:", error);
    return NextResponse.json({ error: error?.message || "Failed to mark order as picked" }, { status: 500 });
  }
}
